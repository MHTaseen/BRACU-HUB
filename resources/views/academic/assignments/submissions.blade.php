                ...
                @if($sub)
                    <div style="border-top: 1px solid var(--glass-border); margin-top: 1rem; padding-top: 1rem;">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 1rem;">
                            ...
                            {{-- Grading Panel (AJAX) --}}
                            <div style="background: rgba(168, 85, 247, 0.05); border: 1px solid rgba(168, 85, 247, 0.2); padding: 1rem; border-radius: 8px; min-width: 200px; margin-bottom: 1rem;">
                        ...
                        <div class="grade-panel">
                            ...
                            <form method="POST" action="{{ route('submissions.grade', $sub->id) }}" style="display: flex; align-items: center; gap: 0.75rem; flex-wrap: wrap;">
                                @csrf
                                @method('PATCH')
                                <label for="marks-upstream-{{ $sub->id }}">
                                    {{ $isGraded ? 'Update Marks' : 'Enter Marks' }}
                                </label>
                                <input
                                    type="number"
                                    name="marks_obtained"
                                    id="marks-upstream-{{ $sub->id }}"
                                    class="grade-input"
                                    step="0.5"
                                    min="0"
                                ...
                                <button type="submit" class="btn-grade">
                                    {{ $isGraded ? '✏️ Update' : '✅ Save Marks (Form)' }}
                                </button>
                            </form>
                        </div>
                    </div>
